...
void setup() 
{
	...
	digitalWrite(output4, HIGH);								// remove 5v from pin 5 so relay in1 will be 0v and this make relay OFF
	...
	buttonState1 = digitalRead(wifiButton1);    				// read the state of the pushbutton value:
			...
			if (buttonState1 == LOW) 
				{                                     			// pin 5 is pressed and connected to GND so it will be LOW
					Serial.println("digitalRead(wifiButton1)");
					digitalWrite(output4, HIGH);               	// remove 5v from pin 7 so relay in1 will be 0v and this make relay on
					digitalWrite(GREEN_LED_PIN, HIGH);      	// turn GREEN LED ON
					delay (2000);                             	// wait 5 second
					digitalWrite(GREEN_LED_PIN, LOW);       	// turn GREEN LED OFF
					digitalWrite(output4, LOW);             	// add 5v to arduino pin 7 so relay in1 will be 5v and this make relay off
					Serial.println("digitalRead(wifiButton1)");
				}
	...
	buttonState2 = digitalRead(homeButton2);    				// read the state of the pushbutton value:
			...
			if (buttonState2 ==LOW) 
				{                                     			// pin 5 is pressed and connected to GND so it will be LOW
					...
					digitalWrite(output4, LOW);         		// remove 5v from pin 5 so relay in1 will be 0v and this make relay ON
					digitalWrite( GREEN_LED_PIN, LOW );   		// turn GREEN LED ON

...
void unlock()
{
  ...
  digitalWrite(GREEN_LED_PIN, HIGH);							// turn GREEN BUTTON LED ON
  tone(buzzer, 1250);											// Send 1.250 KHz sound signal
  delay(2500);													// delay for 2.5 sec
  noTone(buzzer);												// buzzer off
  digitalWrite(GREEN_LED_PIN, LOW);								// turn GREEN BUTTON LED OFF
...
void loop()
{
  ...
  {
    digitalWrite(buttonState2, HIGH);							// add 5v to arduino pin 4 so relay in1 will be 5v and this make relay off
  }
